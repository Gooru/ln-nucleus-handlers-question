...
class TenantAuthorizer implements Authorizer<AJEntityQuestion> {
  ...
  public ExecutionResult<MessageResponse> authorize(AJEntityQuestion model) {
    ...
    ContentTenantAuthorization authorization = ContentTenantAuthorizationBuilder
        .build(contentTenantTree, userTenantTree,
            ContentTreeAttributes.build(model.isQuestionPublished()));
  ...
  private ExecutionResult<MessageResponse> checkAuthBasedOnParentBeingPublished(
      AJEntityQuestion model,
      TenantTree userTenantTree, TenantTree contentTenantTree) {
    ExecutionResult<MessageResponse> result =
        checkAuthBasedOnParents(model, userTenantTree, contentTenantTree,
            AJEntityQuestion.TABLE_COURSE,
            model.getCourseId());
    if (result.continueProcessing()) {
      return result;
    }
    return checkAuthBasedOnParents(model, userTenantTree, contentTenantTree,
        AJEntityQuestion.TABLE_COLLECTION,
        model.getCollectionId());
  }

  private ExecutionResult<MessageResponse> checkAuthBasedOnParents(AJEntityQuestion model,
      TenantTree userTenantTree,
