...
public class UpdateQuestionScoreHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    if (!AJEntityQuestion.RUBRIC_ASSOCIATION_ALLOWED_TYPES
      ...
      return new ExecutionResult<>(
          MessageResponseFactory
              .createInvalidRequestResponse(RESOURCE_BUNDLE.getString("scoring.not.allowed")),
          ExecutionResult.ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
          ...
          return new ExecutionResult<>(
              MessageResponseFactory
                  .createInternalErrorResponse(
                      RESOURCE_BUNDLE.getString("unable.to.update.scoring")),
              ExecutionResult.ExecutionStatus.FAILED);
        }
    ...
    if (!result) {
      LOGGER.error("Question score store (Rubric creation) failed for question '{}'",
          context.questionId());
      if (rubric.hasErrors()) {
