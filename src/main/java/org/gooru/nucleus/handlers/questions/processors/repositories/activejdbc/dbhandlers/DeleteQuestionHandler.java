  ...
  private ExecutionResult<MessageResponse> adjustAssessmentGrading() {
    ...
    try {
      ...
      if (assessmentId != null && !assessmentTimestampUpdated) {
        ...
        if (rows != 1) {
          LOGGER.warn(
              "update of the assessment timestamp failed for assessment '{}' with question '{}'",
              assessmentId, context.questionId());
          return new ExecutionResult<>(
              MessageResponseFactory
      ...
      LOGGER.warn("Update of the assessment grading failed for assessment '{}' with question '{}'",
          assessmentId,
