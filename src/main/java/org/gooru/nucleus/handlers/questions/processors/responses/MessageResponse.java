  ...
  public static class Builder {
    ...
    public MessageResponse build() {
      ...
      if (this.httpStatus == null || this.status == null) {
        LOGGER.error(
            "Can't create response with invalid status or http status. Will return internal error");
        result = buildErrorResponse();
      } else {
        result = new JsonObject();
        result.put(MessageConstants.MSG_OP_STATUS, this.status)
            .put(MessageConstants.RESP_CONTAINER_MBUS,
    ...
    private JsonObject buildErrorResponse() {
      JsonObject result =
          new JsonObject()
              .put(MessageConstants.MSG_OP_STATUS, MessageConstants.MSG_OP_STATUS_ERROR);
      result.put(MessageConstants.RESP_CONTAINER_MBUS,
          new JsonObject()
              .put(MessageConstants.MSG_HTTP_STATUS, HttpConstants.HttpStatus.ERROR.getCode()).put(
