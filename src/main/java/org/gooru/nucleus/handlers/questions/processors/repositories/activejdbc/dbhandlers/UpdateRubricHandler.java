...
public class UpdateRubricHandler implements DBHandler {
    ...
    if (isRemote && url != null && !url.isEmpty()) {
      ...
      if (isUrlUpdated) {
        ...
        if (duplicates != null && !duplicates.isEmpty()) {
          ...
          return new ExecutionResult<>(
              MessageResponseFactory.createValidationErrorResponse(response),
              ExecutionResult.ExecutionStatus.FAILED);
        }
      }
    }
  ...
  private JsonArray checkDuplicate() {
    ...
    LazyList<AJEntityRubric> duplicateRubrics = AJEntityRubric
        .findBySQL(AJEntityRubric.SELECT_DUPLICATE,
